...
class transaction_model extends CI_Model {
    ...
    public function get_purchaseheader_by_date($datefrom, $dateend, $suplierpurchase){
      ...
      public function get_salesheader_by_date($datefrom, $dateend, $customersales, $sales_id){
        if($customersales == '' && $sales_id == ''){
        ...
        $query = $this->db->query("select * FROM tbl_header_sales LEFT JOIN tbl_header_retur_sales ON tbl_header_sales.sales_faktur_id = tbl_header_retur_sales.retur_sales_faktur_ref JOIN tbl_customer ON tbl_header_sales.sales_customer_id = tbl_customer.customer_id and sales_date between '".$datefrom."' and '".$dateend."' and customer_id = '".$customersales."' order by sales_header_id desc");
             ...
             $query = $this->db->query("select * FROM tbl_header_sales LEFT JOIN tbl_header_retur_sales ON tbl_header_sales.sales_faktur_id = tbl_header_retur_sales.retur_sales_faktur_ref JOIN tbl_customer ON tbl_header_sales.sales_customer_id = tbl_customer.customer_id and sales_date between '".$datefrom."' and '".$dateend."' and sales_sales_name = '".$sales_id."' order by sales_header_id desc");
        }else{
            $query = $this->db->query("select * FROM tbl_header_sales LEFT JOIN tbl_header_retur_sales ON tbl_header_sales.sales_faktur_id = tbl_header_retur_sales.retur_sales_faktur_ref JOIN tbl_customer ON tbl_header_sales.sales_customer_id = tbl_customer.customer_id and sales_date between '".$datefrom."' and '".$dateend."' and sales_sales_name = '".$sales_id."' and customer_id = '".$customersales."' order by sales_header_id desc");
        }
        $result = $query->result();
        return $result;
    }
    ...
    public function check_fakturcode($fakturcode){
        $query = $this->db->query("select * from tbl_header_sales where sales_faktur_id = '".$fakturcode."'");
        $result = $query->result();
        return $result;
    }
